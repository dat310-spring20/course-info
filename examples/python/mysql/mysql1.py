...
def create_db():
    ...
    try:
        conn = mysql.connector.connect(user='root', password='foobarfoo', host='127.0.0.1')
    except mysql.connector.Error as err:
        ...
        cur = conn.cursor()
        try:
            sql = ("CREATE DATABASE dat310")
            cur.execute(sql)
            print("created dat310 database")
        except mysql.connector.Error as err:
            print("Error: {}".format(err.msg))
        finally:
            cur.close()
...
if __name__ == "__main__":
    ...
    try:
        conn = mysql.connector.connect(user='root', password='foobarfoo', host='127.0.0.1', database='dat310')
    except mysql.connector.Error as err:
        if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
            print("Invalid username/password.")
        elif err.errno == errorcode.ER_BAD_DB_ERROR:
